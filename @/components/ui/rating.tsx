...
const unratedColorVariants = cva("text-gray-500 dark:text-gray-400", {
  variants: {
    variant: {
...
const ratedColorVariants = cva("text-gray-300 dark:text-gray-400", {
  variants: {
    variant: {
...
export const Rating = React.forwardRef<HTMLDivElement, RatingProps>(
  (
    ...
    const ratingRatedColor = cn(
      ratedColorVariants({
        variant: unratedColor || "yellow",
        size: size || "md",
      })
    );
    const ratingUnratedColor = cn(
      unratedColorVariants({
        variant: unratedColor || "gray",
        size: size || "md",
      })
    );
    ...
    const ratedIconClasses = cn(
      !readonly
        ? "transition-colors duration-300 ease-in-out cursor-pointer select-none hover:scale-110 dark:hover:scale-110 transform-gpu hover:translate-y-0 dark:hover:translate-y-0 translate-y-0 hover:translate-x-0 dark:hover:translate-x-0 translate-x-0"
        : ""
    );
