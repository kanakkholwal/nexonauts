...
import { AppCard, AppContainer } from 'layouts/app-page';
...
import type { AppTypeViewOnly } from "types/app";
...
export default function Dashboard({
    ...
    const [apps, setApps] = useState<AppTypeViewOnly[]>(defaultApps);
                        ...
                        return (
                            <AppCard className="mb-3 d-flex flex-column justify-content-between " key={app._id}>
                                ...
                                <div className="footer">
                                    <span className="category">{app.categories[0].replaceAll("_", " ")}</span>
...
export async function getServerSideProps(context: GetSessionParams | undefined) {
    ...
    const apps = await App.find({
        ...
        .select('name shortDescription appId path coverImage recommended version membership categories tags developer createdAt status')
        .exec();

    return {
        props: {
            user: session.user as SessionUserType,
            apps: JSON.parse(JSON.stringify(apps)) as AppTypeViewOnly []
        },
    }
}