...
export class TableUtil {
  ...
  modifyCells = (action, format, position = 'at') => {
    ...
    if (!!selection && Range.isCollapsed(selection)) {
      ...
      if (node) {
        ...
        if (
          ((numberOfColumns === 1 && format === 'column') || (numberOfRows === 1 && format === 'row')) &&
          action === 'remove'
        ) {
          this.removeTable();
          return;
  ...
  modifyRow = (action, path, tableNode) => {
    ...
    Transforms.setNodes(
      this.editor,
      { rows: action === 'remove' ? rows - 1 : rows + 1 },
      {
        at: tablePath,
      }
    );
  ...
  modifyColumn = (action, path, tableNode) => {
    const [{ rows, columns }, tablePath] = tableNode;
  
    for (let rowIndex = 0; rowIndex < rows; rowIndex++) {
      const rowPath = [...path.slice(0, -1), rowIndex]; // Path to the current row
      ...
      if (action === 'remove') {
        ...
        Transforms.insertNodes(this.editor, createTableCell(''), {
                at: [...path.slice(0, -1), path[path.length - 1] + 1],
       });
      }
    }
    ...
    Transforms.setNodes(
      this.editor,
      { columns: action === 'remove' ? columns - 1 : columns + 1 },
      {
        at: rowPath,
      }
    );
