...
const CodeRenderer = ({ data,language,format }: Props) => {
  ...
  return (<pre tabIndex={0} className={`language-${language} relative`}>
      <button type="button" className="absolute right-2 top-2 text-slate-600 font-bold  text-xs hover:text-primary"  onClick={(e) =>{
          e.preventDefault();
          e.stopPropagation();
          copy(data,{
            onCopy:() =>{
