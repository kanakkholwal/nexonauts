    ...
    return (
        <div className="w-full p-4 flex justify-around items-center gap-4">
            ...
            <div className="inline-flex items-center gap-1">
                ...
                <span className="inline-block font-semibold text-sm text-foreground">{post.author.name}</span>
            </div>
                <div>
                    <h6 title={post.title}> <Link href={"/blog/posts/"+ post.slug}>{post.title}</Link></h6>
                    <div>
                    <span className="inline-block text-sm font-semibold">{post.labels[0]}</span>
                ...
                <Image src={post.image} width={200} height={120} alt={post.title} className="w-40 h-28" />
        </div>
    )
}