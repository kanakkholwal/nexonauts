...
export default function TOC({ blocks }: { blocks: Block[] }) {
...
export const TOCMarkDown = ({ content }: { content: string }) => {
...
const TOCListMD = ({ headings }: { headings: Heading[] }) => {
  return (
    <ol>
...
const remapBlocks = (blocks: Block[], level: number = 2): Block[] => {
  ...
  return topLevelBlocks.map((block) => {
    ...
    const endIndex = blocks.findIndex(
      (b, index) => index > startIndex && b.data.level === level
    );
