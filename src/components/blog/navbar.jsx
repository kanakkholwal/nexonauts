...
const NavMenu = styled.ul`
  ...
  @media (max-width: 992px) {
    ...
    padding: 0.75rem;
    box-shadow: rgba(84, 32, 162, 0.11) 0px 8px 15px;
    transition: all 0.3s cubic-bezier(0.645, 0.045, 0.355, 1);
    overflow: hidden;
    max-width: 300px;
    ${NavLink},${NavItem} {
...
export default function NavBarSearch() {
  ...
  const handleSearch = async (e) => {
    ...
    await fetch('')
      ...
      .then((json) => {
        setResult(
          json.map(({ title, short_description, link, service_name }) => {
  ...
  return (
    <SearchWrapper onSubmit={handleSearch}>
      <SearchIcon active={isSearchActive} onClick={handleSearch} type="submit">
        <CgSearch />
      </SearchIcon>
      <SearchInput
      ...
      <DropDownWrapper open={isDropOpen}>
        ...
        {result.length > 0 ? (
          ...
          result.length === 0 &&
          error.length === 0 &&
          loading === false && <h6>No result Found !!!</h6>
        )}
      </DropDownWrapper>
    </SearchWrapper>
  );
