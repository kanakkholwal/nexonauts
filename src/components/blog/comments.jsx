...
export default function Comments({ post }) {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    [`/api/posts/${post._id}/comments/all`, {}],
    ([url, data]) => fetchData(url, data)
  );
      ...
      >
        ...
        <CommentFooter>
          ...
          {comment.replies.length > 0 ? (
            <span className="info">{comment.replies.length} Replies</span>
          ) : (
            <span className="info">No reply</span>
          )}
        </CommentFooter>

        <RepliesWrapper ariaLabelledBy={comment._id}>
          <CommentFormComponent
            canComment={showReplyForm}
            ...
            handleCommentSubmit={(event) => handleReply(event, comment._id)}
            session={session}
            name={replyName}
            email={replyEmail}
        ...
        >
          ...
          <div className="elements">
            ...
            <div className="d-flex justify-content-start align-items-center g-2 flex-wrap">
              <Button type="submit" low="true" size="sm" nature="blog-theme">
                Submit Comment
              </Button>
              <Button
                type="reset"
