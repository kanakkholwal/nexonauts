...
export const buttonVariants = tv({
	...
	variants: {
		variant: {
			...
			secondary: "bg-surface-strong text-ink hover:bg-hairline aria-expanded:bg-hairline",
			// Faint ghost — nav/icon triggers.
			ghost: "bg-transparent text-ink hover:bg-surface-strong/70 aria-expanded:bg-surface-strong",
			...
			// Warning — soft tint.
			warning: "bg-warning/12 text-warning hover:bg-warning/20 focus-visible:ring-warning/30",
			// Glass — for dark-hero overlays. Translucent white with hairline.
			glass: "bg-white/10 text-on-dark border-white/15 backdrop-blur-md hover:bg-white/15",
			// Inverted glass for light-canvas hero use.
			"glass-ink": "bg-ink/5 text-ink border-hairline backdrop-blur-md hover:bg-ink/10",
			// Atmospheric soft pills — one per gradient stop. Decoration-friendly.
			soft: "bg-surface-strong text-ink hover:bg-hairline",
			"soft-mint": "bg-gradient-mint/30 text-ink hover:bg-gradient-mint/45",
			"soft-peach": "bg-gradient-peach/30 text-ink hover:bg-gradient-peach/45",
			"soft-lavender": "bg-gradient-lavender/35 text-ink hover:bg-gradient-lavender/50",
			"soft-sky": "bg-gradient-sky/30 text-ink hover:bg-gradient-sky/45",
			"soft-rose": "bg-gradient-rose/30 text-ink hover:bg-gradient-rose/45",
			// Light-on-dark pill for use inside surface-dark/hero bands.
			"on-dark": "bg-on-dark text-ink hover:bg-on-dark/90"
		},
		size: {
			default:
				"h-10 gap-1.5 px-5 rounded-pill has-data-[icon=inline-end]:pr-4 has-data-[icon=inline-start]:pl-4",
			...
			"icon-xs":
				"size-7 rounded-pill in-data-[slot=button-group]:rounded-md [&_svg:not([class*='size-'])]:size-3",
