	...
	{#snippet children({ thumbItems })}
		...
		{#each thumbItems as thumb (thumb.index)}
			<SliderPrimitive.Thumb
				...
				index={thumb.index}
				class="border-ring ring-ring relative size-3 rounded-full border bg-white transition-[color,box-shadow] after:absolute after:-inset-2 hover:ring-2 focus-visible:ring-2 focus-visible:outline-hidden active:ring-2 block shrink-0 select-none disabled:pointer-events-none disabled:opacity-50"
			/>
		{/each}
	{/snippet}
</SliderPrimitive.Root>
