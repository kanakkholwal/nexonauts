...
export default function OutputTab({ 
    ...
    const [finalSubmitText, setFinalSubmitText] = useState<any | null>('Submitting....');


    async function SubmitApp(appObject: AppType) {

        const data = JSON.stringify(appObject);
        const config = {
            headers: {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const response = await axios.post(`https://api.app.textify.ai/app/submit`, data, config);
                if (response.status === 201) {
                    toast.success("App submitted successfully");
                    resolve(response.data);

                } else {
                    setFinalSubmitText([' Error publishing your app, Please try again later'])
    ...
    function UpdateApp(appObject: AppType) {
        const data = JSON.stringify(appObject);
        const config = {
            headers: {
                "accept": "application/json",
        ...
        return new Promise(async (resolve, reject) => {
            try {
                const response = await axios.put(`https://api.app.textify.ai/app/${appObject._id}`, data, config);
    ...
    function HandlePublish() {
        const prompt = replaceWords(app.config?.prompt ?? "", app.formFlow.inputs.map((input) => input.id));
        const objectRes = CreatePostaleObject(prompt);
        console.log(objectRes);
        if (mode === "edit") {
            toast.promise(UpdateApp(objectRes), {
                loading: "Updating App",
                success: "App Updated successfully",
                error: err => err ?? "Something went wrong please try again"
            })

            return;
        }
        else {
            toast.promise(SubmitApp(objectRes), {
                loading: "Submitting App",
    ...
    function CreatePostaleObject(prompt: string): AppType {

        const app = {...builderData}
        return {
            ...app,
    ...
    return (<> <div className=" w-full mb-2">
            ...
            {/* <Select onValueChange={(value) => {
                updateBuilderData({ ...builderData, 
                    individual_app_features: {
                        ...builderData.individual_app_features ,
                        output_type: value ?? "plain_text"
                    }
                })
            }}
            defaultValue={builderData.individual_app_features?.output_type ?? "plain_text"}
            >
        ...
        <div className='flex justify-between items-center gap-2 mt-2'>
            ...
            {/* <Switch
                ...
                checked={builderData.individual_app_features?.output_save_to_db}

                onCheckedChange={(checked) => {
                    updateBuilderData({ ...builderData, 
                        individual_app_features: {
                            ...builderData.individual_app_features ,
                            output_save_to_db: checked
                        }
                    })
