...
export default function Header() {
    ...
    return (
        <header className={"fixed left-0 top-0 w-full z-9999 py-7 lg:py-0 z-50 " + (isScrolled ? " bg-slate-50 backdrop-blur-lg shadow" : "")} ref={headerRef}>
                    ...
                    <nav>
                        <ul className="flex lg:items-center flex-col lg:flex-row gap-5 lg:gap-2">
                            {menu.map((item: MenuLinkType, index: number) => {
                                return (<li className="nav__menu grow relative lg:py-7" key={index}>
                                    <Link href={item.link} className="relative flex gap-1 items-center text-sm py-1.5 px-4 border border-transparent  hover:bg-primary/10 rounded-full  hover:border-primary/50">
                                        {item.name}
