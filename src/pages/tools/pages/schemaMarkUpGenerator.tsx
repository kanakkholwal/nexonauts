                ...
                <Select onValueChange={(value) => {
                    ...
                    <SelectContent>
                        {options.map((option, index) => {
                            return <SelectItem key={index} value={option.value}>{option.label}</SelectItem>
                        })}
                    </SelectContent>
                </Select>
                ...
                {options.map((option, index) => {
                    if (option.value === schema) {
                        if (option.Component === undefined)
                            return <Card key={index}>Not Available yet</Card>
                        return <Card key={index}>
                            <CardHeader>
                                <CardTitle>
                                    Generating <strong className="mx-1">{option.label}</strong> Schema
                                </CardTitle>
                            </CardHeader>
                            <CardContent>
                                <option.Component setCode={setHtml} />
                            </CardContent>
                        </Card>
                    }
                    return null;
