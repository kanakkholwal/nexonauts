...
export default function OutputTab({ user }: { user: SessionUserType }) {
  ...
  async function updateApp(appObject: AppTypeWithId) {
    return new Promise(async (resolve, reject) => {
      await axios
        .put(`/api/apps/update?appId=${appObject.appId}`, {
          appData: appObject,
          userId: user._id,
        })
        .then((response) => {
  ...
  return (
    <>
      ...
      <div className="flex justify-between items-center gap-2 mt-2">
        <Label htmlFor="output_save_to_db">Save to Database</Label>
        <Switch
          id="output_save_to_db"
          name="output_save_to_db"
...
const OUTPUT_TYPES = ['markdown', 'html', 'plain/text'];
