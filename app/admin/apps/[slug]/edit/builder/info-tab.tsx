...
export default function InfoTab() {
    ...
    return (<>
        <div className="flex flex-col gap-2 ">
            ...
            <Input id="title" name="title" value={name}
        ...
        <div className="flex flex-col gap-2 ">
            ...
            <Input id="slug" name="slug" value={slug}
                ...
                onChange={(e) => {
                    ...
                    useAppStore.setState({ slug: e.target.value.replaceAll(/ /g, "_") })
                }} />
            <span className="text-xs text-gray-400"> ({process.env.NEXT_PUBLIC_WEBSITE_URL}/apps/{slug.trim().length === 0 ? "your-app-link" : slug.toLowerCase().replaceAll(/ /g, "-").replaceAll("_", "-")})</span>
        </div>
        ...
        <div className="flex flex-col gap-2 ">
            ...
            <p className="text-xs text-gray-400 font-semibold">
                Characters: {" "}
                <span className={(description.length >= 300 ? "text-green-500" : description.length <= 200 ? "text-red-500" : "text-primary")}>
                ...
                return <div className="flex items-center space-x-2" key={category}>
                    <Checkbox
                        ...
                        onCheckedChange={(value) => {
                            ...
                            if (value) {
                                useAppStore.setState({ categories: [...categories, category] })
                            } else {
                                useAppStore.setState({ categories: categories.filter((cat) => cat !== category) })
                            }
                        }}
                    />
