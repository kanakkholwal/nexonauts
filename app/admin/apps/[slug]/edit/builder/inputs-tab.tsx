...
export default function InputTab() {
  ...
  const handleDragOver = (e: React.DragEvent, index: number) => {
    ...
    const items = [...inputs];
    const draggedItem =
      items[draggedItemIndex !== null ? draggedItemIndex : index];
  ...
  const updateBuilderDataAndResetInput = (updatedInput: inputType) => {
    ...
    const index = updatedInputs.findIndex(
      (item) => item.field_id === updatedInput.field_id
    );
    if (index !== -1) {
    ...
    <>
      <div className="flex flex-col gap-2">
        <div className="flex flex-col justify-between items-center p-5 bg-slate-100 dark:bg-slate-900 rounded-md">
          <div className="flex flex-row justify-between items-center w-full">
            <Label htmlFor="add_field">Input Field</Label>
            <Select
              onValueChange={(value) => {
                console.log(value);
                setInput({
          ...
          <div className="flex flex-row justify-between items-center w-full gap-2 my-2">
            <Input
              id="label"
              type="text"
              placeholder="@Field Name (eg. user_name)"
          ...
          {input.field_type === 'text_input' ||
          input.field_type === 'text_multiline' ? (
            <Input
              id="placeholder"
              type="text"
              placeholder="Field Placeholder"
              value={input.field_placeholder}
              onChange={(e) => {
                setInput({
                  ...input,
          ...
          {input.field_type === 'number_input' ? (
            <div className="flex w-full flex-row gap-1 my-2">
              <Input
                id="min"
                type="number"
                placeholder="Min"
          ...
          input.field_type === 'checkbox' ? (
            <div>
              ...
              {input.options?.map((option, index) => {
                return (
                  <div
                    className="flex flex-row justify-between items-center w-full gap-2 my-2"
                    key={index}
                  >
                    <Input
                      id={`option_${index}`}
          ...
          <div className="flex flex-row justify-center items-center my-2">
            <Button
              ...
              onClick={() => {
                ...
                if (
                  !INPUT_TYPES.find((type) => type.value === input.field_type)
                ) {
          ...
          <div className="flex flex-col gap-1 my-2 border  w-full bg-gray-50 dark:bg-slate-950 p-1  rounded-md  ">
            {inputs?.map((item, index) => {
              return (
                <div
                  className="flex flex-row justify-between items-center w-full gap-2 border-b-1 border-slate-200"
                  key={index}
                  draggable
                >
            ...
            {inputs.length === 0 ? (
              <span className="px-3 py-2 text-sm font-semibold mx-auto bg-slate-100 dark:bg-slate-900 rounded">
                No Input Fields
              </span>
            ) : null}
          </div>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex flex-row justify-between items-center">
          <Label htmlFor="generate_btn">Generation Button Text</Label>
          <Select
            onValueChange={(value) => {
              useAppStore.setState((state) => {
                return {
