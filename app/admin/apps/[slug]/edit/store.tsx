...
type Actions = {
  ...
  UPDATE_DESCRIPTION: (
    description: AppTypeWithId['description']
  ) => Partial<AppTypeWithId>;
  UPDATE_SLUG: (slug: AppTypeWithId['slug']) => Partial<AppTypeWithId>;
  ADD_CATEGORY: (
    category: keyof AppTypeWithId['categories']
  ) => Partial<AppTypeWithId>;
  ...
  SET_BANNER: (
    bannerImage: AppTypeWithId['bannerImage']
  ) => Partial<AppTypeWithId>;
  UPDATE_MENU_TYPE: (
    menuType: formFlowType['menuType']
  ) => Partial<AppTypeWithId>;
  ...
  UPDATE_PROMPT: (
    prompt: ConfigurationType['prompt']
  ) => Partial<AppTypeWithId>;
  UPDATE_CONFIG_PARAMS: (
    configParams: ConfigurationType['params']
  ) => Partial<AppTypeWithId>;
};
type Action = {
  type: keyof Actions;
...
const appReducer = (state: State, action: Action) => {
  switch (action.type) {
    ...
    case 'UPDATE_MENU_TYPE':
      return {
        ...state,
        formFlow: { ...state.formFlow, menuType: action.payload },
      };
    case 'ADD_INPUT':
      return {
        ...state,
        formFlow: {
          ...state.formFlow,
          inputs: [...state.formFlow.inputs, action.payload],
        },
      };
    case 'REMOVE_INPUT':
      return {
        ...
        formFlow: {
          ...state.formFlow,
          inputs: state.formFlow.inputs.filter((_, i) => i !== action.payload),
        },
      };
    case 'UPDATE_INPUT':
      ...
      return {
        ...
        formFlow: {
          ...state.formFlow,
          inputs: state.formFlow.inputs.map((obj) => {
    ...
    case 'UPDATE_OUTPUT':
      return {
        ...state,
        formFlow: { ...state.formFlow, output: action.payload },
      };
    case 'UPDATE_MODEL':
