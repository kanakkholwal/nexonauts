            ...
            <div className="grid grid-cols-1 gap-4 w-full">
                ...
                <div className="max-w-sm">
                    ...
                    <Input id="link" name="link" type="url"
                        value={tool?.link}
                        placeholder="link of the tool" disabled={loading}
                        onChange={(e) => {
                        ...
                        {available_categories.map((category, index) => {
                            return <div key={"category_" + index}
                                ...
                                <Checkbox
                                    ...
                                    checked={tool.categories.map((cat) => cat.slug).includes(category.slug)}
                                    disabled={loading}
                                    onCheckedChange={(checked) => {
                                        if (checked) {
                                            // check if category already exists
                                            if (!tool.categories.map((cat) => cat.slug).includes(category.slug)) {
                                                // setCategories([...categories, category]);
                                                useFormStore.setState({ tool: { ...tool, categories: [...tool.categories, category] } })
                                            }
                                        } else {
                                            useFormStore.setState({ tool: { ...tool, categories: tool.categories.filter((cat) => cat.slug !== category.slug) } })
                                        }
                                    }}
                                />
            ...
            <div className="grid grid-cols-1 gap-4 flex-0 glassmorphism p-5 rounded-xl w-full md:min-w-[24rem] md:max-w-md mx-auto">
                ...
                <div className="grid items-center gap-1.5 py-4">
                    <div className="flex justify-between items-center flex-wrap gap-2">
                        <Label htmlFor="coverImage" className="mb-0">Cover Image</Label>
                        <UploadImage
                    ...
                    >
                        ...
                        <SelectContent>
                            {['free', 'paid', 'freemium', 'one_time_license', 'subscription', 'open_source', 'other'].map((pricing_type) => {
                                return <SelectItem key={`tool_pricing_${pricing_type}`} value={pricing_type} className="captialize">{pricing_type}</SelectItem>
                            })}
                        </SelectContent>
                    </Select>
                </div>
