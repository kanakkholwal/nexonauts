    ...
    return (<div className="space-y-6 my-5">
        ...
        <div className="grid grid-cols-1 gap-2">
            {links.map((link, index) => {
                return <div key={index} className="grid grid-cols-1 gap-1 glassmorphism p-3 rounded-xl">
                    <h3 className="text-lg font-bold">{link.title}</h3>
                    <Link href={link.url} className="text-sm font-medium hover:underline text-primary" target='_blank'>
                        Go to {link.title}
                    </Link>
