...
// Function to count apps and calculate percent growth
export async function apps_CountAndGrowth(timeInterval: string): Promise<{ count: number; growth: number; trend: 'increase' | 'decrease' | 'stable' }> {
    await dbConnect();
    let startTime: Date;
    ...
    const prevCount = await AppModel.countDocuments({ createdAt: { $gte: prevTimeIntervalStartTime, $lt: startTime } });
...
export async function tools_CountAndGrowth(timeInterval: string): Promise<{ count: number; growth: number; trend: 'increase' | 'decrease' | 'stable' }> {
    await dbConnect();
    let startTime: Date;
    ...
    const prevCount = await PublicToolModel.countDocuments({ createdAt: { $gte: prevTimeIntervalStartTime, $lt: startTime } });
