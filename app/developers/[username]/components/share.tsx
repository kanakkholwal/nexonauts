        ...
        <DialogContent>
            ...
            <div className="flex flex-col items-center justify-center space-y-2">
                <Avatar className="w-20 h-20 shadow-lg">
                    <AvatarImage src={profile.profilePicture} alt={profile.username} width={320} height={320} className="w-20 h-20" />
                    <AvatarFallback className="w-20 h-20 uppercase text-2xl">{profile.username[0] + profile.username[1]}</AvatarFallback>
                </Avatar>
            ...
            <div className="flex flex-row items-center justify-center flex-wrap gap-2 w-full">
                {socials.map(social => <Button key={social.name} className="rounded-full" size="icon" variant="slate" onClick={() => window.open(social.url, '_blank')} title={`Share on ${social.name}`}><social.icon className="w-4 h-4" /></Button>)}
                <Button className="rounded-full" variant="slate" size="icon" onClick={() => share()}  title={`More Options to share`}>
