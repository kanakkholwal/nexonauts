                ...
                {profiles?.map((profile) => {
                    return <div key={profile._id}>
                        <Link href={`/devs/${profile.username}`} className="p-1">
                            <Card className="rounded-2xl backdrop-blur backdrop-saturate bg-opacity-75 bg-white border relative p-4" variant="glass">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <Avatar className="w-20 h-20 border">
                                            <AvatarImage src={profile.user.profilePicture} alt={profile.username} width={320} height={320} />
                                            <AvatarFallback className="uppercase text-xl">{profile.username[0] + profile.username[1]}</AvatarFallback>
                                        </Avatar>
                                        <div>
