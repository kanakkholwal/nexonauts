    ...
    return (
        ...
        <Dialog key={"followers-modal"}>
            <DialogTrigger asChild>
                <span className="text-gray-600 hover:text-foreground cursor-pointer">{developer.followers.length} followers</span>
                </DialogTrigger>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>
                ...
                <ul className="flex flex-col gap-2">
                    {developer.followers.map(follower =>  {
                        ...
                        return <li key={"follower_" + follower.id}>
                        <div className="flex items-center justify-between space-x-4">
                            <div className="flex items-center space-x-4">
                                <Avatar>
                                    <AvatarImage src={follower.profilePicture} alt={follower.username} />
                                    <AvatarFallback className="capitalize">{follower.username[0] + follower.username[1]}</AvatarFallback>
                                </Avatar>
                                <div>
