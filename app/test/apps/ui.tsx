    ...
    <>
      <section className="max-w-[1170px] mt-8 mx-auto px-4 sm:px-8 xl:px-0 z-10 mb-10">
        <div className="mt-5 group shadow-lg relative aiSearch-border-gredient border border-[#8f59ec] rounded-lg lg:py-2 lg:pl-6 p-4 flex gap-1 justify-center md:justify-between lg:flex-row items-center">
          ...
          <Button
            className="hero-button-gradient"
            onClick={() => {
              handleSearch(query);
            }}
          >
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-7 pt-7">
          {filteredApps.map((app) => {
            const Category = CATEGORIES.find((category) =>
              app.categories.includes(category.value)
            );

            return (
              <Card
                key={app._id}
                className="py-5 flex flex-col justify-between group"
              >
                ...
                <CardContent className="px-5 flex flex-col items-start">
                  <CardDescription className="text-md font-medium text-slate-600">
                ...
                <CardFooter>
                  <Link
                    href={app.path}
                    className="inline-flex items-center text-white font-semibold hero-button-gradient relative rounded-lg  text-sm  gap-1.5 py-3 px-6 shadow-button hover:button-gradient-hover hover:shadow-none"
                  >
