...
export default async function Page({ params }: { params: { slug: string } }) {
    ...
    return <main className="p-10 flex min-h-screen gap-2 w-full justify-between flex-wrap items-start relative z-0">
        <div className="max-w-5xl">
            ...
            {session?.user ? <AppplicationView app={app} user={session?.user} /> : <div className="relative w-full aspect-video flex justify-center items-center bg-slate-100 my-5">
                    ...
                    <div className="absolute inset-0  backdrop-blur-sm flex justify-center items-center z-10">
                        <Link href={"/login?continue=" + encodeURI("/apps/"+ app.slug)} className="w-full max-w-xs flex items-center justify-center border border-solid border-primary rounded-md p-2 bg-slate-100">
        ...
        <div className="grow">
                <Tabs defaultValue="my_review" className="w-full">
                    ...
                    <TabsContent value="my_review">
                        {session?.user ? <PostReview app={app} user={session?.user} /> : <Link href={"/login?continue=" + encodeURI("/apps/"+ app.slug)} className="w-full flex items-center justify-center border border-solid border-primary rounded-md p-2 bg-slate-100">
