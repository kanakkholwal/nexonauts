        ...
        <div className="grid grid-cols-1 gap-4">
          {step === 0 && (<>
            <div className="flex flex-col items-start">
              ...
              <Input
                name="username" id="username" placeholder="Username" value={profile.username}
                className="w-full" onChange={(e) => setProfile({ ...profile, username: e.target.value })}
              />
            ...
            <div className="flex flex-col items-start">
              ...
              <Textarea
                name="bio" id="bio" placeholder="Bio" value={profile.bio}
                className="w-full" onChange={(e) => setProfile({ ...profile, bio: e.target.value })}
              />
              <p className="text-[0.8rem] text-muted-foreground">
          ...
          {step === 1 && (<>
            ...
            {SOCIALS.map((key, index) => {
              ...
              return (<div className="relative" key={`socials.${index}.url`}>
            ...
            <div className="flex justify-between  items-center mt-5 gap-2">
              <Button onClick={() => {
                if (profile.socials.filter((social) => social.url.trim().length > 0).length === 0) {
          ...
          {step === 2 && (<>
            ...
            <div className="flex flex-col items-start gap-2">
              ...
              <TagInput placeholder="Enter a topic" className="w-full"
          ...
          {step === 3 ? <>
            <div className="flex justify-center items-center gap-2 flex-col">
              {status === "success" ? <>
                ...
                <h1 className="text-3xl font-semibold text-center">Profile Created!</h1>
                <p className="text-center text-sm">
                  Your profile is live now  <Link href={`/devs/${profile.username}`} target="_blank" className="text-primary hover:underline">@{profile.username}</Link>
                </p>
              </> : null}
              {status === "error" ? <>
                <BiError className="text-red-500 h-16 w-16" />
                <h1 className="text-3xl font-semibold text-center">Error Creating Profile</h1>
              </> : null}
              {status === "loading" ? <>
                <Player
                  autoplay
                ...
                />
                <h1 className="text-3xl font-semibold text-center">Creating Profile...</h1>
              </> : null}
