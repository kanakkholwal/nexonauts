...
export async function getUserIntegrationData(platform: string) {
    ...
    await dbConnect();
    const user = await User.findById(session.user._id).select(`integrations.${platform}`).exec();
    const integrationData = JSON.parse(JSON.stringify(user.integrations[platform]));
    return integrationData;
}
export async function saveAccessToken(options: Record<string, any>,platform:string) {
    "use server";
    try {
