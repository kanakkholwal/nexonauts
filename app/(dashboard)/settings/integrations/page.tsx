            ...
            <Suspense fallback={<div className="flex items-center justify-center h-40"><CgSpinnerTwo className="animate-spin h-10 w-10 text-primary" /> </div>}>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {INTEGRATIONS.map((integration) => {
                        return <Link href={`/settings/integrations/${integration}`} key={integration} className="flex items-center border text-card-foreground backdrop-blur bg-white dark:bg-gray-600/30 shadow p-4 rounded-xl flex-col justify-center">
